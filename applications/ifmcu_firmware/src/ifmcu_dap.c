...
LOG_MODULE_REGISTER(ifmcu_dap, CONFIG_IFMCU_DAP_LOG_LEVEL);
...
extern struct k_work_delayable nrf91_reset_work;
extern struct k_work_delayable ifmcu_reset_work;
extern struct k_work_delayable ifmcu_bootloader_work;
 ...
 */
static size_t dap_execute_vendor_cmd(struct dap_link_context *const dap_link_ctx, uint8_t *in,
			      uint8_t *out)
{
	...
	if (in[0] == ID_DAP_VENDOR_IFMCU_VERSION) {
		const size_t VERSION_STRING_LEN = 62U;

		int len = snprintf(out + 2, VERSION_STRING_LEN,
			"Boot: %u.%u.%u, App: " APP_VERSION_STRING "-g" STRINGIFY(APP_BUILD_VERSION),
			((NRF_TIMER2->CC[0] >> 16) & 0xFF),
			((NRF_TIMER2->CC[0] >> 8) & 0xFF),
			(NRF_TIMER2->CC[0] & 0xFF));
				  ...
				  struct net_buf *buf, int err)
{
	...
	if (atomic_test_bit(&data->state, SAMPLE_FUNCTION_ENABLED) && err == 0) {
		...
		if (ep == dap_func_get_bulk_in(c_data)) {
			...
			len = dap_execute_vendor_cmd(data->dap_link_ctx,

...
LOG_MODULE_REGISTER(MODULE, CONFIG_IFMCU_BULK_LOG_LEVEL);
...
#if defined(CONFIG_IFMCU_CMSIS_DAP_VENDOR_COMMANDS)
	len = dap_execute_vendor_cmd(buf->data, tx_buf);
#else
	len = dap_execute_cmd(buf->data, tx_buf);
#endif /* defined(CONFIG_IFMCU_CMSIS_DAP_VENDOR_COMMANDS) */
